...
ADOPTIQ_BUILD = "36"  # Round 63 / cut Build36 to close the R62 hot spot #1 (corpus_indexer / corpus_bootstrap helper duplication) and the long-carried CI-alignment deferral (R18-NEXT-004 / R20-NEXT-005 / R22-NEXT-CI / R62 deferral). Three changes ship: (1) extracted _safe_log_info + _exit_log_streams_open implementations to a new top-level _logging_helpers.py so corpus_bootstrap.py and corpus_indexer.py both delegate via thin wrappers (single source of truth; the propagate-default drift between the two copies is also unified to True, matching logging.Logger.__init__); (2) added tests/test_round63_corpus_indexer_logger_parity.py with the parity regression-count guard the R62 acceptance section flagged as a known gap; (3) .github/workflows/build.yml::quality-checks now runs `make verify` instead of `pytest -q` (with `pip install ruff bandit pip-audit` added so the dev-only quality tools are CI-installed). Net pytest delta +8 (3655 -> 3663; one existing CI-gate test updated rather than added). R62 source (Quit-button syslog + comprehensive Action plans XLSX row + R61 closed-stream gate) carries forward unchanged.
