...
ADOPTIQ_BUILD = "35"  # Round 62 / cut Build35 to close the three R61 follow-on items the R61 acceptance section called out as hot spots: (1) closed-stream gate broadened module-wide via _safe_log_info helper in BOTH corpus_bootstrap.py (11 sites) AND corpus_indexer.py (5 sites; helper duplicated to avoid circular import); (2) Console.app / `log show --process AdoptIQ` now sees the Quit-button SIGTERM activity via _emit_macos_syslog helper wired into api_shutdown() AND _trigger_shutdown_sigterm() (best-effort /usr/bin/logger call, ~10 ms, skipped on non-darwin and in TESTING); (3) comprehensive XLSX Summary now carries a deterministic ("Action plans (open)", N) row sourced from canonical_metrics.count_open_action_plans(ab_df) so the supervisor harness has a structured-data anchor for action_plans regardless of LLM narrative phrasing. All three were called out in the R61 handoff hot spots; net pytest delta +29 (3626 -> 3655). R60 Quit button + SIGTERM shutdown + R61 update_version_pc footgun fix carry forward.
