"""Phase 4.2 MED: OAuth failure truncates response.text + logs digest only.
...
def test_marker_cisco_internal_integrations_py() -> None:
    ...
    assert 'Round 8 / Phase 4.2' in src, 'Round 8 marker missing in cisco_internal_integrations.py: Round 8 / Phase 4.2'
    assert 'truncate' in src, 'Round 8 pattern missing in cisco_internal_integrations.py: truncate'
