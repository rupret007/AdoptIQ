...
def test_config_hardcoded_share_url_is_documented():
    """Defense-in-depth: when the personal URL appears as a string
    literal in ``config.py``, it MUST sit inside the
    ``ADOPTIQ_CORPUS_SHARE_URL`` definition (not in any other
    config field) and a comment within ~40 lines must document why
    it's hardcoded.  This catches a future engineer who copies the
    ...
    for idx, line in enumerate(body_lines):
        ...
        if "jestory_cisco_com" not in stripped:
            continue
        # Only count quoted string literals.
        import re
        if re.search(r"['\"][^'\"]*jestory_cisco_com[^'\"]*['\"]", stripped):
            hits.append(idx)
    assert hits, (
        "Round 35 expects the canonical share URL to appear as a "
        "string literal in config.py (it is the hardcoded source "
        "of the AdoptIQ Knowledge Corpus)."
    )
    assert len(hits) == 1, (
        "Personal Cisco URL appeared in multiple non-comment "
