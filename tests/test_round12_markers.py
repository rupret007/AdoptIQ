...
def test_marker_phase_1_1_feature_requests_arr_currency() -> None:
    ...
    assert_in_source(src, 'Round 12 / Phase 1.1', label='src')
    assert_in_source(src, 'analyze_feature_requests', label='src')


def test_marker_phase_1_2_arr_impact_currency_contract() -> None:
    src = _read('app_simple.py')
    assert_in_source(src, 'Round 12 / Phase 1.2', label='src')
    assert_in_source(src, 'calculate_arr_impact_for_issues', label='src')


def test_marker_phase_1_3_briefing_mixed_currency_account_id() -> None:
    src = _read('adoptiq_backend.py')
    assert_in_source(src, 'Round 12 / Phase 1.3', label='src')


def test_marker_phase_1_4_portfolio_intel_multicurrency_top5() -> None:
    src = _read('adoptiq_backend.py')
    assert_in_source(src, 'Round 12 / Phase 1.4', label='src')


def test_marker_phase_1_5_estimate_arr_currency_label() -> None:
    src = _read('app_simple.py')
    assert_in_source(src, 'Round 12 / Phase 1.5', label='src')
...
def test_marker_phase_2_1_briefing_recent_barriers_utc() -> None:
    src = _read('adoptiq_backend.py')
    assert_in_source(src, 'Round 12 / Phase 2.1', label='src')


def test_marker_phase_2_2_csone_scope_filter_utc() -> None:
    src = _read('adoptiq_backend.py')
    assert_in_source(src, 'Round 12 / Phase 2.2', label='src')


def test_marker_phase_2_3_incident_newest_coalesce() -> None:
    src = _read('incident_storage.py')
    assert_in_source(src, 'Round 12 / Phase 2.3', label='src')
    assert_in_source(src, 'COALESCE', label='src')


def test_marker_phase_2_4_barrier_velocity_week_key() -> None:
    src = _read('adoptiq_backend.py')
    assert_in_source(src, 'Round 12 / Phase 2.4', label='src')
...
def test_marker_phase_4_2_analyze_html_console_log_pii() -> None:
    ...
    assert_in_source(src, 'Round 12 / Phase 4.2', label='src')
...
def test_marker_phase_4_4_external_intel_format_number() -> None:
    ...
    assert_in_source(src, 'Round 12 / Phase 4.4', label='src')
    assert_in_source(src, 'format_number', label='src')


def test_marker_phase_4_5_history_html_format_number() -> None:
    src = _read('templates/history.html')
    assert_in_source(src, 'Round 12 / Phase 4.5', label='src')
    assert_in_source(src, 'format_number', label='src')
...
def test_marker_phase_6_1_historical_distribution_sample_flag() -> None:
    src = _read('adoptiq_backend.py')
    assert_in_source(src, 'Round 12 / Phase 6.1', label='src')
...
def test_marker_phase_6_2_cross_trends_severity_canonical_sheet() -> None:
    ...
    # combined marker.
    assert_in_source(src, 'Round 12 / Phase 6.2' in src or 'Round 12 / Phase 6.2 + 6.3', label='src')
...
def test_marker_phase_6_3_cross_trends_total_rows_canonical_sheet() -> None:
    ...
    assert_in_source(src, 'Round 12 / Phase 6.3' in src or 'Round 12 / Phase 6.2 + 6.3', label='src')


def test_marker_phase_6_4_ask_ai_historical_truncation_tags() -> None:
    src = _read('app_simple.py')
    assert_in_source(src, 'Round 12 / Phase 6.4', label='src')


def test_marker_phase_6_5_sample_subjects_rename() -> None:
    src = _read('adoptiq_backend.py')
    assert_in_source(src, 'Round 12 / Phase 6.5', label='src')


def test_marker_phase_6_6_ask_ai_barrier_aging_arr() -> None:
    src = _read('app_simple.py')
    assert_in_source(src, 'Round 12 / Phase 6.6', label='src')
...
def test_marker_phase_7_1_renewal_customer_info_order_by() -> None:
    ...
    assert_in_source(src, 'Round 12 / Phase 7.1', label='src')
    assert_in_source(src, 'ORDER BY', label='src')


def test_marker_phase_7_2_enhanced_snowflake_insights_order_by() -> None:
    src = _read('enhanced_snowflake_insights.py')
    assert_in_source(src, 'Round 12 / Phase 7.2', label='src')
    assert_in_source(src, 'ORDER BY', label='src')


def test_marker_phase_7_3_json_lite_stable_sort() -> None:
    src = _read('adoptiq_backend.py')
    assert_in_source(src, 'Round 12 / Phase 7.3', label='src')
...
def test_marker_phase_8_2_fig_text_clip_margin() -> None:
    src = _read('adoptiq_backend.py')
    assert_in_source(src, 'Round 12 / Phase 8.2', label='src')


def test_marker_phase_8_3_renewal_panel_1_neutral_palette() -> None:
    src = _read('app_simple.py')
    assert_in_source(src, 'Round 12 / Phase 8.3', label='src')


def test_marker_phase_8_4_portfolio_pie_fixed_band_order() -> None:
    src = _read('adoptiq_backend.py')
    assert_in_source(src, 'Round 12 / Phase 8.4', label='src')
...
def test_marker_phase_11_1_round_percent_migration() -> None:
    src = _read('adoptiq_backend.py')
    assert_in_source(src, 'Round 12 / Phase 11.1', label='src')
    assert_in_source(src, '_r12_round_percent' in src or 'round_percent', label='src')
...
def test_marker_phase_11_3_api_success_vs_ok_shim() -> None:
    src = _read('app_simple.py')
    assert_in_source(src, 'Round 12 / Phase 11.3', label='src')
...
def test_marker_phase_11_6_debug_log_redaction() -> None:
    src = _read('adoptiq_backend.py')
    assert_in_source(src, 'Round 12 / Phase 11.6', label='src')


def test_marker_phase_11_7_fuzzy_fold_error_flag() -> None:
    src = _read('canonical_metrics.py')
    assert_in_source(src, 'Round 12 / Phase 11.7', label='src')
    assert_in_source(src, 'get_fold_fuzzy_degradation_count', label='src')
