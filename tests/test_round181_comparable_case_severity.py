"""Round 181: TAC-fallback likely-next only from comparable-severity cases.

``cases.severity`` is already persisted. Theme-matched P1 and P4 paths
must not publish the same case-basis likely-next. Barrier and pulse
bases stay the Round 176 SSoT. Existing Ask AI / Customer 360 /
Historical Context cards inherit via the R177 view. Fixtures only;
not a new report or page.
"""

from __future__ import annotations

import csv
import shutil
from pathlib import Path

import pytest

import corpus_retriever as cr
import report_corpus_context as rcc
from corpus_indexer import index_folder, open_corpus_db
from source_shape_utils import assert_in_source

PEER_METHOD = "Rotated service token and updated documentation for SSO setup."
FIXTURES = Path(__file__).resolve().parent / "fixtures" / "round17"
ROOT = Path(__file__).resolve().parents[1]

VIEW_KEYS = frozenset(
    {
        "status",
        "insufficient_reason",
        "insufficient_copy",
        "next_step",
        "likely_next",
        "likely_next_label",
        "method",
        "evidence_line",
        "peer_accounts",
        "method_peers",
        "closed_n",
        "open_n",
        "basis",
        "honesty_label",
        "ready_for_live_cisco",
        "source_id",
        "theme",
        "technology",
    }
)


def _write_csv(path: Path, rows: list[dict[str, str]]) -> None:
    fieldnames = list(rows[0].keys())
    with path.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(rows)


def _peer_barrier(
    customer: str,
    *,
    suffix: str,
    ab_status: str = "",
    resolution: str = PEER_METHOD,
) -> dict[str, str]:
    return {
        "customer_name": customer,
        "SUBJECT_C": f"Authentication SSO login errors {suffix}",
        "SEVERITY_C": "Critical",
        "AB_STATUS_C": ab_status,
        "ID": f"AB-{suffix}",
        "technology": "security",
        "theme": "authentication",
        "resolution": resolution,
    }


def _peer_case(
    customer: str,
    *,
    suffix: str,
    status: str = "Closed",
    severity: str = "P2",
    opened: str = "2026-01-01T00:00:00Z",
    closed: str = "2026-01-10T00:00:00Z",
) -> dict[str, str]:
    closed_at = closed if status == "Closed" else ""
    return {
        "customer_name": customer,
        "Case Number": f"PEER-{suffix}",
        "Title": f"Authentication failure during {suffix} onboarding",
        "Severity": severity,
        "Case Status": status,
        "Date/Time Opened": opened,
        "Date/Time Closed": closed_at,
        "Resolution Summary": "Closed after token rotation.",
        "technology": "security",
    }


def _index_corpus(
    tmp_path: Path,
    extra_rows: dict[str, list[dict[str, str]]] | None = None,
    *,
    copy_bundled_barriers: bool = False,
    copy_bundled_cases: bool = True,
) -> object:
    """Index a temp corpus. Round 181 tests stay self-contained.

    Do not import the R175/R176/R177 suites — they pull app_simple /
    decision_report_delivery and break collection on thin VMs.
    Default skips bundled barriers so Closed AB_STATUS_C cannot steal
    the case-fallback path under test.
    """
    corpus_root = tmp_path / "corpus"
    corpus_root.mkdir(parents=True, exist_ok=True)
    for name in (
        "synthetic_cases.csv",
        "synthetic_barriers.csv",
        "synthetic_pulse.csv",
    ):
        if name == "synthetic_barriers.csv" and not copy_bundled_barriers:
            continue
        if name == "synthetic_cases.csv" and not copy_bundled_cases:
            continue
        shutil.copy2(FIXTURES / name, corpus_root / name)
    extras = extra_rows or {}
    if extras.get("barriers"):
        _write_csv(corpus_root / "peer_barriers.csv", extras["barriers"])
    if extras.get("cases"):
        _write_csv(corpus_root / "peer_cases.csv", extras["cases"])
    if extras.get("pulse"):
        _write_csv(corpus_root / "peer_pulse.csv", extras["pulse"])
    connection = open_corpus_db(tmp_path / "corpus.db")
    index_folder(connection, corpus_root)
    cr.configure_connection(connection)
    return connection


def _evidence(**overrides: object) -> cr.PeerGuidanceEvidence:
    payload: dict[str, object] = {
        "theme": "authentication",
        "technology": "security",
        "peer_customer_count": 3,
        "resolved_peer_count": 2,
        "dominant_method_text": PEER_METHOD,
        "dominant_method_peers": 2,
        "closed_peer_count": 2,
        "open_peer_count": 1,
        "close_time_median_days": 12.0,
        "pulse_recovered_count": 0,
        "pulse_worsened_count": 0,
        "likely_next": "closure",
        "next_step": "apply that observed method to the current open work next",
        "evidence_sufficient": True,
        "method_closed_peer_count": 2,
        "method_open_peer_count": 0,
        "method_pulse_recovered_count": 0,
        "method_pulse_worsened_count": 0,
        "method_barrier_closed_peer_count": 2,
        "method_barrier_open_peer_count": 0,
        "likely_next_basis": "barrier",
    }
    payload.update(overrides)
    return cr.PeerGuidanceEvidence(**payload)  # type: ignore[arg-type]


@pytest.fixture(autouse=True)
def _clear_corpus_connection():
    cr.configure_connection(None)
    yield
    cr.configure_connection(None)


@pytest.mark.parametrize(
    ("raw", "band"),
    [
        ("Critical", "critical"),
        ("P1", "critical"),
        ("sev1", "critical"),
        ("1", "critical"),
        ("High", "high"),
        ("P2", "high"),
        ("2", "high"),
        ("Medium", "medium"),
        ("moderate", "medium"),
        ("P3", "medium"),
        ("Low", "low"),
        ("P4", "low"),
        ("4", "low"),
        ("informational", "low"),
        ("", ""),
        ("unknown", ""),
        ("10", ""),
        ("P10", ""),
    ],
)
def test_peer_severity_band_is_exact(raw: str, band: str) -> None:
    assert cr._peer_severity_band(raw) == band
    assert cr._peer_severity_band("Critical") != cr._peer_severity_band("High")
    assert cr._peer_severity_band("P1") != cr._peer_severity_band("P2")


def test_unanimous_p2_case_fallback_still_publishes_closure(tmp_path: Path) -> None:
    connection = _index_corpus(
        tmp_path,
        {
            "barriers": [
                _peer_barrier("Peer A", suffix="A", ab_status=""),
                _peer_barrier("Peer B", suffix="B", ab_status=""),
            ],
            "cases": [
                _peer_case("Peer A", suffix="A", severity="P2"),
                _peer_case("Peer B", suffix="B", severity="P2"),
            ],
        },
    )
    try:
        evidence = cr.get_peer_guidance_evidence(
            "authentication",
            "security",
            exclude_customer="Synthetic Zeta",
            target_case_severity="",
        )
        assert evidence.likely_next == "closure"
        assert evidence.likely_next_basis == "case"
        assert evidence.evidence_sufficient is True
        assert evidence.incomparable_case_severity is False
        assert evidence.comparable_case_severity_band == "high"
        assert evidence.comparable_case_method_peer_count == 2
        view = rcc.build_peer_guidance_view(evidence)
        assert set(view) == VIEW_KEYS
        assert view["status"] == "actionable"
        assert view["basis"] == "case"
        assert view["ready_for_live_cisco"] is False
    finally:
        connection.close()


def test_unknown_case_severity_keeps_pre_r181_behavior(tmp_path: Path) -> None:
    connection = _index_corpus(
        tmp_path,
        {
            "barriers": [
                _peer_barrier("Peer A", suffix="A", ab_status=""),
                _peer_barrier("Peer B", suffix="B", ab_status=""),
            ],
            "cases": [
                _peer_case("Peer A", suffix="A", severity=""),
                _peer_case("Peer B", suffix="B", severity=""),
            ],
        },
        copy_bundled_cases=False,
    )
    try:
        evidence = cr.get_peer_guidance_evidence(
            "authentication",
            "security",
            exclude_customer="Synthetic Zeta",
            target_case_severity="",
        )
        assert evidence.likely_next == "closure"
        assert evidence.likely_next_basis == "case"
        assert evidence.incomparable_case_severity is False
        assert evidence.comparable_case_severity_band == ""
        assert evidence.comparable_case_method_peer_count == 2
    finally:
        connection.close()


def test_mixed_p1_and_p4_withholds_case_basis_likely_next(tmp_path: Path) -> None:
    connection = _index_corpus(
        tmp_path,
        {
            "barriers": [
                _peer_barrier("Peer A", suffix="A", ab_status=""),
                _peer_barrier("Peer B", suffix="B", ab_status=""),
            ],
            "cases": [
                _peer_case("Peer A", suffix="A", severity="P1"),
                _peer_case("Peer B", suffix="B", severity="P4"),
            ],
        },
    )
    try:
        evidence = cr.get_peer_guidance_evidence(
            "authentication",
            "security",
            exclude_customer="Synthetic Zeta",
            target_case_severity="",
        )
        assert evidence.dominant_method_text == PEER_METHOD
        assert evidence.dominant_method_peers == 2
        assert evidence.method_closed_peer_count == 0
        assert evidence.likely_next == "insufficient"
        assert evidence.likely_next_basis == ""
        assert evidence.next_step == ""
        assert evidence.incomparable_case_severity is True
        assert evidence.case_severity_comparable is False
        view = rcc.build_peer_guidance_view(evidence)
        assert set(view) == VIEW_KEYS
        assert view["status"] == "method_only"
        assert view["insufficient_reason"] == "incomparable_case_severity"
        assert "comparable-severity" in str(view["insufficient_copy"])
        assert "support-case" in str(view["insufficient_copy"])
        assert view["likely_next"] == ""
        assert view["next_step"] == ""
        assert view["ready_for_live_cisco"] is False
        public = rcc.public_peer_guidance_view(view)
        assert public["insufficient_reason"] == "incomparable_case_severity"
        assert public["ready_for_live_cisco"] is False
        lines = rcc.format_peer_guidance_scan_lines(evidence)
        assert any("comparable-severity" in line for line in lines)
        assert not any(line.startswith("Next step:") for line in lines)
    finally:
        connection.close()


def test_critical_case_is_not_high_case(tmp_path: Path) -> None:
    connection = _index_corpus(
        tmp_path,
        {
            "barriers": [
                _peer_barrier("Peer A", suffix="A", ab_status=""),
                _peer_barrier("Peer B", suffix="B", ab_status=""),
            ],
            "cases": [
                _peer_case("Peer A", suffix="A", severity="P1"),
                _peer_case("Peer B", suffix="B", severity="P2"),
            ],
        },
    )
    try:
        evidence = cr.get_peer_guidance_evidence(
            "authentication",
            "security",
            exclude_customer="Synthetic Zeta",
            target_case_severity="",
        )
        assert evidence.incomparable_case_severity is True
        assert evidence.likely_next == "insufficient"
        assert evidence.likely_next_basis == ""
    finally:
        connection.close()


def test_p1_and_critical_are_the_same_case_band(tmp_path: Path) -> None:
    connection = _index_corpus(
        tmp_path,
        {
            "barriers": [
                _peer_barrier("Peer A", suffix="A", ab_status=""),
                _peer_barrier("Peer B", suffix="B", ab_status=""),
            ],
            "cases": [
                _peer_case("Peer A", suffix="A", severity="P1"),
                _peer_case("Peer B", suffix="B", severity="Critical"),
            ],
        },
    )
    try:
        evidence = cr.get_peer_guidance_evidence(
            "authentication",
            "security",
            exclude_customer="Synthetic Zeta",
            target_case_severity="",
        )
        assert evidence.incomparable_case_severity is False
        assert evidence.comparable_case_severity_band == "critical"
        assert evidence.likely_next == "closure"
        assert evidence.likely_next_basis == "case"
    finally:
        connection.close()


def test_target_case_mismatch_withholds_likely_next(tmp_path: Path) -> None:
    connection = _index_corpus(
        tmp_path,
        {
            "barriers": [
                _peer_barrier("Peer A", suffix="A", ab_status=""),
                _peer_barrier("Peer B", suffix="B", ab_status=""),
            ],
            "cases": [
                _peer_case("Peer A", suffix="A", severity="P4"),
                _peer_case("Peer B", suffix="B", severity="P4"),
            ],
        },
    )
    try:
        # Bundled Synthetic Alpha authentication TAC is P2 — auto-lookup
        # must fail closed against Low (P4) peers.
        evidence = cr.get_peer_guidance_evidence(
            "authentication",
            "security",
            exclude_customer="Synthetic Alpha",
        )
        assert evidence.dominant_method_peers == 2
        assert evidence.incomparable_case_severity is True
        assert evidence.likely_next == "insufficient"
        assert evidence.likely_next_basis == ""
        view = rcc.public_peer_guidance_view(rcc.build_peer_guidance_view(evidence))
        assert view["status"] == "method_only"
        assert view["insufficient_reason"] == "incomparable_case_severity"
        assert view["ready_for_live_cisco"] is False
    finally:
        connection.close()


def test_unknown_case_peers_excluded_when_one_band_is_known(tmp_path: Path) -> None:
    connection = _index_corpus(
        tmp_path,
        {
            "barriers": [
                _peer_barrier("Peer A", suffix="A", ab_status=""),
                _peer_barrier("Peer B", suffix="B", ab_status=""),
            ],
            "cases": [
                _peer_case("Peer A", suffix="A", severity="P1"),
                _peer_case("Peer B", suffix="B", severity=""),
            ],
        },
        copy_bundled_cases=False,
    )
    try:
        evidence = cr.get_peer_guidance_evidence(
            "authentication",
            "security",
            exclude_customer="Synthetic Zeta",
            target_case_severity="",
        )
        assert evidence.incomparable_case_severity is False
        assert evidence.comparable_case_severity_band == "critical"
        assert evidence.comparable_case_method_peer_count == 1
        assert evidence.method_closed_peer_count == 1
        assert evidence.likely_next == "insufficient"
        assert evidence.dominant_method_text == PEER_METHOD
        view = rcc.build_peer_guidance_view(evidence)
        assert view["status"] == "method_only"
        assert view["insufficient_reason"] == "mixed_evidence"
    finally:
        connection.close()


def test_barrier_basis_still_wins_when_cases_are_mixed(tmp_path: Path) -> None:
    connection = _index_corpus(
        tmp_path,
        {
            "barriers": [
                _peer_barrier("Peer A", suffix="A", ab_status="Closed"),
                _peer_barrier("Peer B", suffix="B", ab_status="Closed"),
            ],
            "cases": [
                _peer_case("Peer A", suffix="A", severity="P1"),
                _peer_case("Peer B", suffix="B", severity="P4"),
            ],
        },
    )
    try:
        evidence = cr.get_peer_guidance_evidence(
            "authentication",
            "security",
            exclude_customer="Synthetic Zeta",
            target_case_severity="",
        )
        assert evidence.incomparable_case_severity is True
        assert evidence.method_barrier_closed_peer_count == 2
        assert evidence.method_closed_peer_count == 0
        assert evidence.likely_next == "closure"
        assert evidence.likely_next_basis == "barrier"
        view = rcc.build_peer_guidance_view(evidence)
        assert view["status"] == "actionable"
        assert view["basis"] == "barrier"
        assert view["likely_next"] == "closure"
        assert view["ready_for_live_cisco"] is False
    finally:
        connection.close()


def test_mixed_open_closed_same_identity_still_contributes_no_case_outcome(
    tmp_path: Path,
) -> None:
    connection = _index_corpus(
        tmp_path,
        {
            "barriers": [
                _peer_barrier("Peer A", suffix="A", ab_status=""),
                _peer_barrier("Peer B", suffix="B", ab_status=""),
            ],
            "cases": [
                _peer_case(
                    "Peer A",
                    suffix="A-open",
                    status="Open",
                    severity="P2",
                ),
                {
                    **_peer_case(
                        "Peer A",
                        suffix="A-closed",
                        status="Closed",
                        severity="P2",
                    ),
                    "Case Number": "PEER-A-open",
                },
                _peer_case("Peer B", suffix="B", severity="P2"),
            ],
        },
        copy_bundled_cases=False,
    )
    try:
        evidence = cr.get_peer_guidance_evidence(
            "authentication",
            "security",
            exclude_customer="Synthetic Zeta",
            target_case_severity="",
        )
        assert evidence.method_closed_peer_count == 1
        assert evidence.likely_next == "insufficient"
        assert evidence.likely_next_basis == ""
    finally:
        connection.close()


def test_receipt_fingerprint_includes_case_severity_band(tmp_path: Path) -> None:
    connection = _index_corpus(
        tmp_path,
        {
            "barriers": [
                _peer_barrier("Peer A", suffix="A", ab_status=""),
                _peer_barrier("Peer B", suffix="B", ab_status=""),
            ],
            "cases": [
                _peer_case("Peer A", suffix="A", severity="P2"),
                _peer_case("Peer B", suffix="B", severity="P2"),
            ],
        },
    )
    try:
        matched = cr.get_peer_guidance_evidence(
            "authentication",
            "security",
            exclude_customer="Synthetic Zeta",
            target_case_severity="",
        )
        mixed = cr.PeerGuidanceEvidence(
            theme=matched.theme,
            technology=matched.technology,
            peer_customer_count=matched.peer_customer_count,
            resolved_peer_count=matched.resolved_peer_count,
            dominant_method_text=matched.dominant_method_text,
            dominant_method_peers=matched.dominant_method_peers,
            closed_peer_count=matched.closed_peer_count,
            open_peer_count=matched.open_peer_count,
            close_time_median_days=matched.close_time_median_days,
            pulse_recovered_count=matched.pulse_recovered_count,
            pulse_worsened_count=matched.pulse_worsened_count,
            likely_next="insufficient",
            next_step="",
            evidence_sufficient=True,
            exclusion_digest=matched.exclusion_digest,
            incomparable_case_severity=True,
            comparable_case_severity_band="",
            comparable_case_method_peer_count=0,
        )
        sid_ok = rcc.peer_guidance_source_id(matched)
        sid_mixed = rcc.peer_guidance_source_id(mixed)
        assert sid_ok.startswith("CORPUS:PG-")
        assert sid_mixed.startswith("CORPUS:PG-")
        assert sid_ok != sid_mixed
    finally:
        connection.close()


def test_constructed_evidence_defaults_remain_comparable() -> None:
    view = rcc.build_peer_guidance_view(_evidence())
    assert view["status"] == "actionable"
    assert view["ready_for_live_cisco"] is False


def test_no_will_or_live_cisco_in_case_severity_copy() -> None:
    evidence = _evidence(
        likely_next="insufficient",
        next_step="",
        likely_next_basis="",
        incomparable_case_severity=True,
        method_closed_peer_count=0,
        method_barrier_closed_peer_count=0,
    )
    view = rcc.build_peer_guidance_view(evidence)
    assert view["insufficient_reason"] == "incomparable_case_severity"
    assert "will " not in str(view["insufficient_copy"]).casefold()
    assert view["ready_for_live_cisco"] is False
    copy = str(view["insufficient_copy"])
    # The R175.4 case-id regex false-positives on "case path". Canned
    # operator copy must still reach Word scan lines via the allow-list.
    assert cr._peer_text_leaks_pii(copy) is True
    lines = rcc.format_peer_guidance_scan_lines(evidence)
    assert any("comparable-severity" in line for line in lines)
    assert copy in lines
    assert rcc._R177_HONESTY_LABEL in lines
    assert not any(line.startswith("Next step:") for line in lines)


def test_source_shape_round181_markers() -> None:
    retriever = (ROOT / "corpus_retriever.py").read_text(encoding="utf-8")
    context = (ROOT / "report_corpus_context.py").read_text(encoding="utf-8")
    this_test = Path(__file__).read_text(encoding="utf-8")
    assert_in_source(retriever, "Round 181", label="ret")
    assert_in_source(retriever, "_peer_case_severity_band", label="ret")
    assert_in_source(retriever, "_lookup_target_case_severity_band", label="ret")
    assert_in_source(retriever, "incomparable_case_severity", label="ret")
    assert_in_source(retriever, '"severity"', label="ret-sql")
    assert_in_source(context, "incomparable_case_severity", label="ctx")
    assert_in_source(context, "comparable-severity", label="ctx")
    assert_in_source(context, "_R181_CANNED_SCAN_LINES", label="ctx")
    import_lines = [
        line
        for line in this_test.splitlines()
        if line.lstrip().startswith(("import ", "from "))
    ]
    joined = "\n".join(import_lines)
    assert "test_round175_peer_guidance_knowledge" not in joined
    assert "test_round176_barrier_status_peer_join" not in joined
    assert "test_round177_peer_guidance_surfaces" not in joined
