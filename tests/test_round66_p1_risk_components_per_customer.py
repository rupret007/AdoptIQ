...
def test_risk_components_block_is_present_in_comprehensive_flow(app_simple_source: str) -> None:
    ...
    """
    assert_in_source(app_simple_source, "Round 66 / Pass 2 (B6): per-customer ``Risk_Components``", label='app_simple_source')
    assert_in_source(app_simple_source, "_r66_risk_rows", label='app_simple_source')
    assert_in_source(app_simple_source, 'all_sheets["Risk_Components"]', label='app_simple_source')
...
def test_risk_components_block_sorts_by_score_desc_then_name_asc(app_simple_source: str) -> None:
    ...
    assert_in_source(sort_block, "Risk_Score_0_100", label='sort_block')
    assert_in_source(sort_block, "Customer_Name", label='sort_block')
...
def test_risk_components_block_writes_provenance_row_when_no_profiles(app_simple_source: str) -> None:
    ...
    assert_in_source(block, '"_adoptiq_provenance_row": True', label='block')
    assert_in_source(block, '"AdoptIQ_Status": "EMPTY"', label='block')
    assert_in_source(block, "risk_scoring.compute_customer_risk_profile", label='block')
...
def test_risk_components_block_handles_non_dict_profile_safely(app_simple_source: str) -> None:
    ...
    block_end = app_simple_source.index("xlsx_path = write_excel_workbook(base, all_sheets,")
    block = app_simple_source[block_start:block_end]
    assert_in_source(block, "if not isinstance(_r66_profile, dict):", label='block')
    assert_in_source(block, "continue", label='block')
...
def test_risk_components_block_wraps_in_try_except(app_simple_source: str) -> None:
    ...
    assert_in_source(block, "except Exception as _r66_risk_err:", label='block')
    assert_in_source(block, "logger.debug(", label='block')
...
def test_risk_components_block_emits_top_risk_factor_column(app_simple_source: str) -> None:
    ...
    block_end = app_simple_source.index("xlsx_path = write_excel_workbook(base, all_sheets,")
    block = app_simple_source[block_start:block_end]
    assert_in_source(block, '"Top_Risk_Factor"', label='block')
    assert_in_source(block, '_r66_factors[0]', label='block')
    assert_in_source(block, ":480]", label='block')
...
def test_risk_components_block_does_not_clobber_action_plans(app_simple_source: str) -> None:
    ...
    ap_idx = app_simple_source.index('all_sheets["Action_Plans"]')
...
def test_risk_components_block_uses_expected_band_value_passthrough(app_simple_source: str) -> None:
    ...
    block_end = app_simple_source.index("xlsx_path = write_excel_workbook(base, all_sheets,")
    block = app_simple_source[block_start:block_end]
    assert_in_source(block, '_r66_profile.get("risk_band")', label='block')
    assert_in_source(block, '_r66_profile.get("risk_score_0_100")', label='block')
