...
def peer_guidance_source_id(evidence: object) -> str:
    ...
    try:
        comparable_case_n = int(getattr(evidence, "comparable_case_method_peer_count", 0) or 0)
    except (TypeError, ValueError):
        comparable_case_n = 0
    case_incomp = "1" if bool(getattr(evidence, "incomparable_case_severity", False)) else "0"
    payload = "|".join(
        [
            theme,
            tech,
...
def build_peer_guidance_view(evidence: object) -> dict[str, object]:
    ...
    incomparable = bool(getattr(evidence, "incomparable_severity", False))
    case_incomparable = bool(getattr(evidence, "incomparable_case_severity", False))
    if incomparable:
        reason = "incomparable_severity"
        insufficient_copy = _R177_INSUFFICIENT_COPY[reason]
    elif case_incomparable and likely_raw == "insufficient":
        reason = "incomparable_case_severity"
        insufficient_copy = _R177_INSUFFICIENT_COPY[reason]
    elif target_path == "already_closed":  # Round 179
...
def public_peer_guidance_view(raw: object) -> dict[str, object]:
    ...
    if status == "insufficient":
        ...
        if status == "method_only" and not copy:
            if reason in {"already_lived", "incomparable_severity", "incomparable_case_severity"}:
...
def format_peer_guidance_scan_lines(evidence: object) -> tuple[str, ...]:
    ...
    if next_step:
        ...
        if view.get("insufficient_reason") in {"incomparable_severity", "incomparable_case_severity"}:
            lines.append(str(view.get("insufficient_copy") or ""))
        else:
            lines.append(
...
def select_ranked_peer_guidance(
    *,
    ...
    """
    wanted = _r182_normalize_prefer_theme(prefer_theme)  # strict path filter
    ...
    ranked.sort(
        key=lambda item: (
            ...
            *(str(value) for value in item[0][-2:]),
        )
    )
    return ranked[0][1]
