...
1. ``settings.json`` value managed by ``adoptiq_settings`` (UI-set
   ...
   Round 116 / Build 85: a settings value of the stale ``gpt-5-nano``
   default is healed to the Gemini default at read time UNLESS the
   matching ``*_user_set`` flag is True (a deliberate Preferences
   dropdown pick).  This robustly un-wedges installs that the one-time
   R103/R108/R115 marker migrations could never reach.
...
def _r116_coerce_settings_value(raw: str, model_key: str) -> str:
    """Round 116 / Build 85: heal a stale ``gpt-5-nano`` settings value to the
    Gemini default at read time UNLESS the operator explicitly picked it via
    the Preferences dropdown (the ``*_user_set`` flag in settings.json).

    This is the AUTHORITATIVE runtime heal.  It covers the exact Build 84
    acceptance wedge — an install pinned to ``gpt-5-nano`` with all three
    one-time migration markers (R103/R108/R115) already ``True``, where no
    marker migration can act — and any case where the on-disk self-heal
    (``adoptiq_settings.migrate_round116_model_user_set``) could not persist
    (read-only FS, etc.).  ``model_key`` is ``"report_model_name"`` or
    ``"ask_ai_model_name"``.
    """
    if raw != _R103_STALE_DEFAULT_MODEL:
        return raw
    try:
        import adoptiq_settings as _settings
        if _settings.is_user_set(model_key) is True:
            # Deliberate post-Build-85 nano pick — honor it.
            return raw
    except Exception:  # noqa: BLE001
        # adoptiq_settings unimportable in this fixture: default to the
        # safe heal (Gemini) rather than propagating stale nano.
        pass
    logger.info(
        "model_resolver: Round 116 healed stale settings %s=%s to %s "
        "(no operator user_set flag)",
        model_key, _R103_STALE_DEFAULT_MODEL, _HARDCODED_DEFAULT,
    )
    return _HARDCODED_DEFAULT
...
def get_active_ask_ai_model() -> str:
    ...
    if candidate:
        return _r116_coerce_settings_value(candidate, "ask_ai_model_name")
...
def get_active_report_model() -> str:
    ...
    if candidate:
        return _r116_coerce_settings_value(candidate, "report_model_name")
