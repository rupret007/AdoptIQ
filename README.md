...
#### Quality harness
- **Test suite:** **2445 passed / 2 skipped** (up from 2106 after Round 17) — Rounds 18-28 added 339 new tests across report determinism, multi-currency arithmetic safety, AI grounding, AdoptIQ Intelligence indexing, admin CSRF & audit-mirror hardening, and connectivity-diagnostics namespace cleanups.
...
- **Audit log.** Running findings, fixes, and residual risks are tracked in `QUALITY_AUDIT.md` (Round 14 → Round 28 sections). Older `CODE_REVIEW_*.md` files are frozen point-in-time snapshots; consult `QUALITY_AUDIT.md` for current state.

#### Quality & Hardening — Rounds 18-28
