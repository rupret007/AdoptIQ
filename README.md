...
#### CSOne Knowledge Corpus (Round 17 / 17.1 / 17.2 / 35, opt-in)
- **NEW (Round 35): Native Knowledge Corpus — baked at build time, refreshed daily.** The `AI Projects/AdoptIQ_CSOne_Reports` share is now hardcoded into the build (no per-user URL configuration) and indexed during `build_mac_dmg.sh` via `scripts/bake_corpus.py`. The four ship-ready artifacts (`corpus.db.enc`, `sentinel.json`, `corpus.db.salt`, `corpus.sentinel.lock.json`) ride inside the .app under `Resources/baked_corpus/`. On first launch `corpus_bootstrap` copies them into `~/Library/Application Support/AdoptIQ/knowledge/` (mode `0o600`); subsequent launches leave the user's existing corpus alone. A daemon thread wakes every hour and triggers a Microsoft Graph refresh whenever the user is signed in AND 24 hours have elapsed since the last successful refresh. The encrypted DB is updated atomically (sibling `.tmp` + `os.replace`) so a mid-refresh crash leaves the prior corpus byte-identical. The analyze-page panel is now labelled **"AdoptIQ Knowledge Corpus"** and only carries Connect / Sign-out controls — no URL paste field.
...
### CSOne Knowledge Corpus (Round 35: now native; opt-out via env)

- Round 35 made the corpus a **native** part of every install: the `AI Projects/AdoptIQ_CSOne_Reports` share is indexed at build time and bundled inside the .app, so a freshly-installed AdoptIQ has the corpus loaded immediately on first launch even before the user signs in. The corpus URL is hardcoded in `Config.ADOPTIQ_CORPUS_SHARE_URL` (env-overridable for ops only); there is no user-facing URL paste field.
- A daily refresh worker keeps the corpus current — once the user completes the device-code sign-in (Connect button on the analyze page), the worker wakes every hour and pulls fresh content from the share when the 24-hour window has elapsed. Refreshes use the same atomic write the bake script does, so a mid-refresh crash never corrupts the prior corpus.
...
- Build-time controls: `ADOPTIQ_BAKE_CORPUS=0` skips the bake (useful for dev iteration when you don't have network or MSAL access); the build still produces a working .app, just without a baked corpus, and the runtime falls back to the legacy "first-launch refresh" path. `ADOPTIQ_BAKE_AUTH_MODE=offline_fixture` (with `--offline-fixture <dir>`) is reserved for the test path.
- **Sign in once to SharePoint.** When `ADOPTIQ_SHAREPOINT_ENABLED=true` (default), the analyze-page Intelligence card (Round 33 / Build8) and the Admin Corpus tile both show a "Connect to Microsoft" button on first launch. The flow uses Microsoft Graph device-code OAuth: AdoptIQ shows you a code, you open https://microsoft.com/devicelogin in any browser, paste the code, and AdoptIQ caches the refresh token in the macOS Keychain (with a 0600-permission JSON fallback) so future launches are silent. Press "Refresh SharePoint corpus" any time to re-pull, or **"Sign out"** to drop the cached token (e.g. when switching accounts).
...
- Configure source visibility:
  ...
  - `ADOPTIQ_SHAREPOINT_FOLDER_URL` — the SharePoint share URL. **Round 33 / Build8: no longer defaults to a hardcoded personal Cisco URL** — leave unset and configure it per-user via the analyze-page Intelligence card (`POST /api/settings/sharepoint_url`, persisted to `settings.json`, allow-listed to `https://<tenant>.sharepoint.com/<path>`). The env var is the tenant-wide override for admin-staged deployments. Resolution order: `settings.json` > env > `""` (treated as "not configured" — the analyze-page panel prompts for a URL).
...
#### How do I connect SharePoint? (Round 33 / Build8)

1. Open AdoptIQ and stay on the analyze page (`/`). Find the **AdoptIQ Intelligence** card near the top.
2. The **SharePoint connection** sub-panel shows one of three states:
   - **Not configured** — paste your SharePoint folder URL into the field (e.g. `https://contoso.sharepoint.com/sites/customer-success/Shared%20Documents/AdoptIQ_CSOne_Reports`) and click **Save URL**. Only `https://<tenant>.sharepoint.com/<path>` URLs are accepted; other schemes / hosts are rejected with a 400.
   - **Sign-in required** — click **Connect to Microsoft**. AdoptIQ shows you a one-time device code; open the verification URL in any browser, sign in with your Microsoft work account, and paste the code. The panel auto-detects sign-in completion within a few seconds and starts indexing.
   - **Signed in** — your account is shown. **Sign out** drops the cached refresh token from the macOS Keychain + 0600 fallback file; the next refresh requires a fresh device-code sign-in (use this to switch accounts).
3. The encryption sentinel resolution order (Round 33 / Build8) is: synced OneDrive folder → SharePoint Graph cache → auto-minted local `sentinel.json` (mode 0o600, parent 0o700) under `~/Library/Application Support/AdoptIQ/knowledge/`. SharePoint-only installs (no OneDrive sync) just work — the local sentinel inherits the same per-account hardening as `settings.json` and the OAuth token cache.
4. Endpoints: `POST /api/settings/sharepoint_url`, `POST /api/corpus/sharepoint/signin`, `POST /api/corpus/sharepoint/signout`, `POST /api/corpus/sharepoint/refresh` (CSRF-protected; same dual-path as `/api/corpus/refresh` — Flask-WTF token **or** `X-AdoptIQ-Internal` header).
