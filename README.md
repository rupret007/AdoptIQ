...
#### Testing
- **Test suite:** 451 pytest tests covering validation, data processing, downloads, formatting, status management, error paths, NaN/Inf handling, security guards, advanced analytics, error-response sanitization, XSS hardening, cursor safety, LLM call correctness, cancellation logic, path traversal prevention, input validation, external intel API safety, URL encoding, formula injection defense, upload collision prevention, TOCTOU download guards, report insights eviction, leader report None guards, renewal NaN safety, CSRF tokens, schema version handling, compact formatter None/NaN guards, column existence checks, prompt injection boundaries, template hardening, progress-route XSS escaping, insights filename sanitization, PSIRT URL encoding, LLM response guards, exec-intel None/NaN safety, Ask-AI error sanitization, admin port alignment, runs[0] guards, BST/PSIRT CSRF, and division-by-zero defense.
