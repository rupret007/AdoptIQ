...
#### Testing
- **Test suite:** 391 pytest tests covering validation, data processing, downloads, formatting, status management, error paths, NaN/Inf handling, security guards, advanced analytics, error-response sanitization, XSS hardening, cursor safety, LLM call correctness, cancellation logic, path traversal prevention, input validation, external intel API safety, and URL encoding.
